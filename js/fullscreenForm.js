...
(function(window) {
        ...
        function parse_flodata(flodata) {
            ...
            var cleanstringList = cleanstring.split(' ')
            ...
            if (atList.length == 0 && hashList.length == 0 || atList.length > 1 || hashList.length > 1) {
                ...
                var incorporation = isIncorp(cleanstring);
                var transfer = isTransfer(cleanstring);
                    ...
                    onEndAnimationFn = function(ev) {
                        ...
                        if (self.isLastStep) {
                            ...
                            if (result['type'] == 'transfer') {
                                var fieldnames = [{ 'FLO data': result['flodata'] }, { 'Type': 'Transfer' }, { 'Identification': result['tokenIdentification'] }, { 'Amount': result['tokenAmount'] }, { 'FLO Address': result['address'] }]
                            } else if (result['type'] == 'incorporation') {
                                var fieldnames = [{ 'FLO data': result['flodata'] }, { 'Type': 'Incorporation' }, { 'Identification': result['tokenIdentification'] }, { 'Amount': result['tokenAmount'] }]
                            } else {
